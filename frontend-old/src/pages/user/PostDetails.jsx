...
const PostDetails = () => {
    ...
    const submitReview = async () => {
        try {
            await api.put(`/api/posts/${id}/review`,
                { rating, comment },
                { headers: { Authorization: `Bearer ${token}` } }
            );
    ...
    return (
        <div className="max-w-3xl mx-auto mt-8 bg-white p-8 rounded-lg shadow">
            ...
            <div className="mb-6">
                ...
                {post.type === 'image' ? (
                    <img src={post.content} alt="Evidence" className="max-w-full rounded mt-2" />
                ) : post.type === 'audio' ? (
                    <audio controls src={post.content} className="mt-2" />
                ) : (
                    <p className="mt-2 p-4 bg-gray-100 rounded">{post.content}</p>
                )}
            </div>
